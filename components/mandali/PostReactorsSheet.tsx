...
import { fetchPostReactors, type PostReactor, type ReactionType, REACTION_META, REACTION_ORDER } from '@/lib/mandali';
...
// Post-level equivalent of CommentReactorsSheet -- identical structure,
// swapped to the 5-type devotional set posts use (pranam/bhakti/jnana/
// chardi_kala/shanti) rather than comments' 3, and to fetchPostReactors.
// Instagram/Facebook-style "Who reacted" sheet: displays all reactors
// grouped/filtered by reaction type, with safe author projections and
// safety-state filtering (blocked/muted users excluded).
export function PostReactorsSheet({
  visible,
  postId,
  currentUserId,
  onClose,
  onViewProfile,
}: PostReactorsSheetProps) {
  ...
  useEffect(() => {
    ...
    fetchPostReactors(postId, currentUserId)
      .then((data) => {
        if (!cancelled) {
          setReactors(data);
          setLoading(false);
      ...
      .catch((err) => {
        console.error('[PostReactorsSheet] failed to load reactors', err);
        if (!cancelled) {
          setReactors([]);
          setLoading(false);
        }
      ...
      <View style={{ flex: 1, backgroundColor: COLORS.bottomSheetScrim, justifyContent: 'flex-end' }}>
        ...
        >
          ...
          {reactors.length > 0 && availableTabs.length > 2 ? (
            <View style={{ flexDirection: 'row', gap: 6, paddingVertical: 2, flexWrap: 'wrap' }}>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              subtitle="When fellow seekers react with Pranam, Bhakti, Jnana, Chardi Kala, or Shanti, they appear here."
            />
          ) : (
            <ScrollView
              style={{ maxHeight: 340 }}
